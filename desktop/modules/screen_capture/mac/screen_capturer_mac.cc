...
namespace pear::capture {
    ...
    ScreenCapturerMac::ScreenCapturerMac(std::shared_ptr<CaptureSource> source)
            : ScreenCapturer(std::move(source)) {}

    std::shared_ptr<CaptureFrame> ScreenCapturerMac::Capture() {
        CGImageRef image = CGDisplayCreateImage(source_->id);
        if (image == nullptr) {
